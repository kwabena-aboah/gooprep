...
export const useAuthStore = defineStore('auth',()=>{
  ...
  async function login(email,password){loading.value=true;error.value='';try{const{data}=await apiPost('/auth/token/',{email,password});persist(data);return{ok:true,role:data.user.role}}catch(e){error.value=e.response?.data?.detail||'Invalid email or password.';return{ok:false}}finally{loading.value=false}}
  ...
  async function fetchMe(){try{const{data}=await apiGet('/auth/users/me/');user.value=data;localStorage.setItem('gp_user',JSON.stringify(data));return data}catch{return null}}
