...
export const useNotifStore = defineStore('notifs',()=>{
  ...
  function connectWS(){if(ws)ws.close();try{ws=createWS('notifications/');ws.onmessage=e=>{const msg=JSON.parse(e.data);if(msg.type==='notification'){notifications.value.unshift(msg.data);toast(msg.data.title,'info')}};ws.onerror=()=>{ws=null}}catch{}}
